...
void draw_folding(SDL_Renderer* renderer, AppState* state) {
    ...
    double fold_amount = math_sin(state->animation_phase * 0.05) * 0.5 + 0.5;
    ...
    int x12 = cx + (int)(radius_12 * math_cos(angle_12));
    int y12 = cy + (int)(radius_12 * math_sin(angle_12));
    SDL_RenderDrawLine(renderer, cx, cy, x12, y12);
    
    // 12 o'clock to 3 o'clock
    double angle_3 = 0.0;
    double radius_3 = get_ring_radius(0, base_radius);
    int x3 = cx + (int)(radius_3 * math_cos(angle_3));
    int y3 = cy + (int)(radius_3 * math_sin(angle_3));
