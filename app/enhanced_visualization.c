...
void draw_enhanced_factor_lines(SDL_Renderer* renderer, AppState* state, 
    ...
    for (int h = start_idx; h < end_idx; h++) {
        ...
        int num_x = cx + (int)(num_radius * prime_cos(num_angle_rad));
        int num_y = cy + (int)(num_radius * prime_sin(num_angle_rad));
        ...
        for (int i = 2; i < num; i++) {
            if (num % i == 0) {
                ...
                int factor_x = cx + (int)(factor_radius * prime_cos(factor_angle_rad));
                int factor_y = cy + (int)(factor_radius * prime_sin(factor_angle_rad));
                
                // Calculate angle difference for color intensity
                double angle_diff = prime_fabs(num_angle_deg - factor_angle_deg);
        ...
        if (compact_vector_create(&vec, num, state->clock_ctx) == MATH_SUCCESS) {
            ...
            if (get_precise_clock_position(&vec, &pos, state->clock_ctx) == MATH_SUCCESS) {
                ...
                int base_x = cx + (int)(num_radius * prime_cos(base_angle_rad));
                int base_y = cy + (int)(num_radius * prime_sin(base_angle_rad));
                ...
                for (double t = 0.0; t <= 1.0; t += 0.1) {
                    ...
                    int arc_x = cx + (int)(num_radius * prime_cos(interp_rad));
                    int arc_y = cy + (int)(num_radius * prime_sin(interp_rad));
...
void draw_platonic_solid_overlay(SDL_Renderer* renderer, AppState* state,
    ...
    for (int i = 0; i < solid.num_vertices; i++) {
        ...
        double rotated_x = x * prime_cos(num_angle_rad) - y * prime_sin(num_angle_rad);
        double rotated_y = x * prime_sin(num_angle_rad) + y * prime_cos(num_angle_rad);
    ...
    for (int i = 0; i < solid.num_edges; i++) {
        ...
        double rx1 = x1 * prime_cos(num_angle_rad) - y1 * prime_sin(num_angle_rad);
        double ry1 = x1 * prime_sin(num_angle_rad) + y1 * prime_cos(num_angle_rad);
        double rx2 = x2 * prime_cos(num_angle_rad) - y2 * prime_sin(num_angle_rad);
        double ry2 = x2 * prime_sin(num_angle_rad) + y2 * prime_cos(num_angle_rad);
...
void draw_sphere_hierarchy(SDL_Renderer* renderer, AppState* state,
    ...
    for (int angle = 0; angle < 360; angle += 5) {
        ...
        int x = num_x + (int)(root_radius * prime_cos(rad));
        int y = num_y + (int)(root_radius * prime_sin(rad));
    ...
    if (root->num_children > 0) {
        ...
        for (int i = 0; i < root->num_children && i < 12; i++) {
            ...
            int child_x = num_x + (int)(root_radius * 0.7 * prime_cos(child_angle));
            int child_y = num_y + (int)(root_radius * 0.7 * prime_sin(child_angle));
            
            // Draw child circle
            for (int angle = 0; angle < 360; angle += 10) {
                double rad = angle * PRIME_PI / 180.0;
                int x = child_x + (int)(child_radius * prime_cos(rad));
                int y = child_y + (int)(child_radius * prime_sin(rad));
                SDL_RenderDrawPoint(renderer, x, y);
            }
