...
MathError abacus_add_geometric_fp(const CrystallineAbacus* a,
    ...
    // Perform standard geometric addition
    ...
    CrystallineAbacus* rounded = abacus_new(result->base);
    if (!rounded) {
        return MATH_ERROR_OUT_OF_MEMORY;
    }
    
    err = abacus_round_geometric(result, rounded, precision);
    if (err != MATH_SUCCESS) {
        abacus_free(rounded);
        return err;
    }
    
    // Copy rounded result back
    if (result->beads) free(result->beads);
    if (result->sparse_beads) free(result->sparse_beads);
    
    *result = *rounded;
    free(rounded); // Free the structure but not the data
...
MathError abacus_sub_geometric_fp(const CrystallineAbacus* a,
    ...
    // Perform standard geometric subtraction
...
MathError abacus_mul_geometric_fp(const CrystallineAbacus* a,
    ...
    // Perform standard geometric multiplication
