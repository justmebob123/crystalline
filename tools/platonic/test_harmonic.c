...
int main(void) {
    ...
    if (fabs(gradients[0] - original_grad) > 0.001) {
    ...
    if (fabs(attention[0] - original_attention) > 0.001) {
    ...
    for (uint32_t epoch = 0; epoch < 10; epoch++) {
        ...
        double modulation = cos(2.0 * M_PI * epoch / 10.0);
