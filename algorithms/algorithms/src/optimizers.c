        ...
        case OPTIMIZER_ADAGRAD:
            ...
            for (size_t i = 0; i < num_parameters; i++) {
                ...
                parameters[i] -= state->config.learning_rate * gradients[i] /
                                (math_sqrt(state->squared_gradient[i]) + state->config.epsilon);
            }
...
void optimizer_adam_step(
    ...
    double bias_correction1 = 1.0 - math_pow(beta1, (double)step);
    double bias_correction2 = 1.0 - math_pow(beta2, (double)step);
    ...
    for (size_t i = 0; i < num_parameters; i++) {
        ...
        // Update parameter
        parameters[i] -= learning_rate * m_hat / (math_sqrt(v_hat) + epsilon);
...
void optimizer_rmsprop_step(
    ...
    for (size_t i = 0; i < num_parameters; i++) {
        ...
        // Update parameter
        parameters[i] -= learning_rate * grad / (math_sqrt(squared_gradient[i]) + epsilon);
...
double lr_schedule_step_decay(
    ...
    return initial_lr * math_pow(decay_rate, (double)num_decays);
}
...
double lr_schedule_exponential_decay(
    ...
    return initial_lr * math_exp(-decay_rate * (double)step);
}
...
double lr_schedule_cosine_annealing(
    ...
    double cosine = 0.5 * (1.0 + math_cos(M_PI * progress));
