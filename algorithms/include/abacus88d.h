...
typedef struct {
    ...
    // Thread safety
    pthread_mutex_t layer_lock;         // Thread safety for concurrent access
...
typedef struct {
    ...
    pthread_mutex_t boundary_lock;      // Thread safety for boundary operations
...
typedef struct {
    ...
    pthread_mutex_t global_lock;        // Global lock for cross-layer operations
