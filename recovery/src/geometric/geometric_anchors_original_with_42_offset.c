...
static void generate_tetrahedron_13d(GeometricAnchor* anchors, int start_idx) {
    ...
    for (int v = 0; v < 4; v++) {
        ...
        for (int d = 0; d < 13; d++) {
            ...
            // Position = cos(angle * φ_d) * golden_ratio^d
            anchors[start_idx + v].position[d] = 
                cos(angle * phi_d) * pow(PHI, d % 3);
        }
    }
}
...
static void generate_cube_13d(GeometricAnchor* anchors, int start_idx) {
    ...
    for (int v = 0; v < 8; v++) {
        ...
        for (int d = 0; d < 13; d++) {
            ...
            anchors[start_idx + v].position[d] = 
                (x * cos(phi_d) + y * sin(phi_d) + z * cos(2.0 * phi_d)) / sqrt(3.0);
        }
    }
}
...
static void generate_dodecahedron_13d(GeometricAnchor* anchors, int start_idx) {
    ...
    for (int v = 0; v < 20; v++) {
        ...
        for (int d = 0; d < 13; d++) {
            ...
            anchors[start_idx + v].position[d] = 
                cos(angle * phi_d * PHI) * pow(PHI, d % 5);
        }
    }
}
...
bool verify_pythagorean_triple(
    ...
    double lhs = a*a + b*b;
    double rhs = c*c;
    double error = fabs(lhs - rhs) / (rhs + 1e-10);
...
void hash_Q_to_13d_position(EC_POINT* Q, EC_GROUP* group, double position[13]) {
    ...
    for (int d = 0; d < 13; d++) {
        ...
        position[d] = fmod(x_contrib * TWO_PI + y_contrib * PI, TWO_PI);
    }
...
TetrationAttractor compute_tetration_attractor(uint64_t base, int depth) {
    ...
    for (int i = 1; i < depth; i++) {
        ...
        if (fabs(result - prev) < 1e-6) {
            attractor.converged = true;
            break;
        }
        
        // Prevent overflow
        if (result > 1e15 || isnan(result) || isinf(result)) {
            result = prev;
            break;
        }
    }
...
void bias_toward_attractors(double position[13]) {
    ...
    for (int d = 0; d < 13; d++) {
        if (attractors[d].converged) {
            ...
            double attractor_angle = fmod(attractors[d].value, TWO_PI);
...
double distance_to_nearest_attractor(
    ...
    for (int d = 0; d < 13; d++) {
        if (attractors[d].converged) {
            double attractor_angle = fmod(attractors[d].value, TWO_PI);
            double dist = fabs(position[d] - attractor_angle);
