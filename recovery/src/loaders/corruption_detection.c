    ...
    if (data->vertices && data->num_vertices > 1) {
        ...
        if (vertex_distances) {
            ...
            for (size_t i = 0; i < data->num_vertices; i++) {
                ...
                vertex_distances[i] = sqrt(dx*dx + dy*dy + dz*dz);
            ...
            double std_dev = sqrt(variance / data->num_vertices);
            
            // Count outliers (> 3 standard deviations)
            for (size_t i = 0; i < data->num_vertices; i++) {
                if (fabs(vertex_distances[i] - mean_distance) > 3 * std_dev) {
                    num_outliers++;
                }
            }
...
static CorruptionReport* detect_signal_corruption(SignalData* data) {
    ...
    for (size_t i = 0; i < data->num_samples * data->num_channels; i++) {
        if (fabs(data->samples[i]) < threshold) {
            num_dropout++;
        }
    }
    ...
    for (size_t i = 0; i < data->num_samples * data->num_channels; i++) {
        if (fabs(data->samples[i]) > clip_threshold) {
            num_clipped++;
        }
    }
    ...
    if (num_dropout > total_samples * 0.01) { // More than 1% dropout
        ...
        report->severity = fmin(std_dev / 10.0, 1.0); // Normalize to 0-1
