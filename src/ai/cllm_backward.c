...
void cllm_transformer_layer_backward(CLLMTraining* training, int layer_idx,
    ...
    // FeedForwardLayer* ff = &model->ff_layers[layer_idx];  // Unused, commented out
    ...
    // Backward through layer norm (post-attention)
    // Note: This is simplified - full implementation needs attention outputs
...
void cllm_backward_complete(CLLMTraining* training) {
    ...
    printf("Backward pass: Computing gradients...\n");
...
/**
 * Complete backward pass through entire model (NEW IMPLEMENTATION)
 * 
 * @param training Training state
