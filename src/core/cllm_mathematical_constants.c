...
void cllm_init_mathematical_constants(MathematicalConstantsBigFixed* constants) {
    ...
    big_fixed_from_double(constants->pi, M_PI);
    
    // Initialize 2π
    constants->two_pi = big_fixed_create(128);
    big_fixed_from_double(constants->two_pi, 2.0 * M_PI);
    
    // Initialize π/12
    constants->pi_over_12 = big_fixed_create(128);
    big_fixed_from_double(constants->pi_over_12, M_PI / 12.0);
    
    // Initialize 2π/(12·ln3)
    constants->two_pi_over_12_ln3 = big_fixed_create(128);
    big_fixed_from_double(constants->two_pi_over_12_ln3, (2.0 * M_PI) / (12.0 * LN_3));
}
...
int cllm_validate_mathematical_constants(void) {
    ...
    double lambda_error = fabs(lambda - expected_lambda);
