...
void test_known_phonemes() {
    ...
    assert(prime_fabs(get_phonetic_value("dub") - 3.0) < EPSILON);
    assert(prime_fabs(get_phonetic_value("knbt") - 3.0) < EPSILON);
    assert(prime_fabs(get_phonetic_value("k'anchay") - 3.0) < EPSILON);
    assert(prime_fabs(get_phonetic_value("kub") - 3.0) < EPSILON);
...
void test_unknown_phonemes() {
    ...
    assert(prime_fabs(get_phonetic_value("unknown") - 3.0) < EPSILON);
    assert(prime_fabs(get_phonetic_value("xyz") - 3.0) < EPSILON);
    assert(prime_fabs(get_phonetic_value("") - 3.0) < EPSILON);
...
void test_null_input() {
    ...
    assert(prime_fabs(get_phonetic_value(NULL) - 3.0) < EPSILON);
...
void test_phonetic_iteration() {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        assert(name != NULL);
        assert(prime_fabs(value - 3.0) < EPSILON);  // All current values are 3.0
    ...
    assert(prime_fabs(get_phonetic_value_by_index(count) - 0.0) < EPSILON);
